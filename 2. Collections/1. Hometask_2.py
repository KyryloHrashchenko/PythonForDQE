...
for key in duplicated_keys:  # Loop for finding max value for duplicated key and then filling it to duplicated_dict
    ...
    for i, my_dict in enumerate(dict_list, start=1):  # Here we are indexing our dictionaries
...
for my_dict in dict_list:  # Combining all dictionaries from dict_list
...
for key in duplicated_keynames:  # for each key from duplicated_keynames
    unique_dict.pop(key, None)  # delete all keys from unique_dict, which exists in duplicated_keynames

final_dict = {**duplicated_dict, **unique_dict}  # Merging duplicated_dict and unique_dict to final_dict
