from random import randint # importing module for randoming numbers
...
def bubble_sort(array): # Creating function for bubble sorting
    n = len(array) # Creating variable that is equal of array lenght
    for i in range(n):
        already_sorted = True # Create a flag that will allow the function to terminate early if there is nothing left to sort
        for j in range(n - i - 1):
            if array[j] > array[j + 1]: # If the item you are looking at is greater than its adjacent value, then swap them
                array[j], array[j + 1] = array[j + 1], array[j]
                already_sorted = False  # This flag is needed to not stop the code prematurely
        if already_sorted: # If there were no swaps during the last iteration, the array is already sorted and we can stop the code
            break
    return array
