...
class FileCopier:
    ...
    def start(self):
        ...
        if file_path.endswith('.json'):
            parser = JsonParser()
            parser.parse_and_copy(file_path)
...
class JsonParser:
    def parse_and_copy(self, json_file):
        data = self.parse(json_file)
        if data:
            formatted_text = self.format_data(data)
            output_file = input("Enter the file name where the text will be copied: ")
            ...
            if num_lines_choice.lower() == 'all':
                if FileCopier().copy_to_file(formatted_text.split('\n'), output_file):  # Split text into lines
                    os.remove(json_file)
                    print("The original file was successfully deleted.")
...
class XmlParser(JsonParser):
    def parse_and_copy(self, xml_file):
        data = self.parse(xml_file)
        if data:
            formatted_text = self.format_data(data)
            output_file = input("Enter the file name where the text will be copied: ")
            num_lines_choice = input("How many lines to copy from XML? (Type 'all' for all lines, 'custom' for custom number): ")
            if num_lines_choice.lower() == 'all':
                if FileCopier().copy_to_file(formatted_text.split('\n'), output_file):  # Split text into lines
                    os.remove(xml_file)
                    print("The original file was successfully deleted.")
    ...
    def parse(xml_file):
        try:
            ...
            for item in root.findall('item'):
                ...
                data['news_feed'].append(item_data)
            return data
        except Exception as e:
